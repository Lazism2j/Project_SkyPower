...
namespace KYG_skyPower
{
    ...
    public class InventoryManagerSO : ScriptableObject
    {
        ...
        public List<IInventoryItemAdapter> GetItemsByType(string type)
        {
            return inventory
                .Select(x => x.itemData)
                .Where(x => x is EquipmentData ed && ed.Equip_Type == type)
                .ToList();
        }

        public List<IInventoryItemAdapter> GetItemsBySlotType(EquipmentSlotType slotType)
        {
            return inventory
                .Select(x => x.itemData)
                .Where(x => x is EquipmentData ed && ed.GetSlotType() == slotType)
                .ToList();
        }
