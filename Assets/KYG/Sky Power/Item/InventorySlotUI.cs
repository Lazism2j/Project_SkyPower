...
{
    public class InventorySlotUI : MonoBehaviour // 인벤토리 슬롯 UI 클래스
    {
        public Image icon; // 아이콘 이미지 컴포넌트
        public Text nameText; // 아이템 이름 텍스트 컴포넌트
        public Button equipButton; // 장비 버튼 컴포넌트
        public Image highlight; // 장비 강조 표시 이미지 컴포넌트

        public void Set(EquipmentData data, bool isEquipped, Action<EquipmentData> onEquip) // 슬롯 설정 메서드 (장비 데이터, 장비 여부, 장비 클릭 시 콜백)
        {
            icon.sprite = data.GetIcon(); // 아이콘 설정 (아이콘은 Resources 폴더에서 로드)
            nameText.text = data.GetName(); // 아이템 이름 설정
